...
export default function ParentPayments() {
  ...
  const fetchPayments = async () => {
    ...
    } catch { toast.error('Erreur lors du chargement des paiements'); } finally { setLoading(false); }
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {loading ? (
        <div className="space-y-6"><Skeleton className="h-40 w-full" /><Skeleton className="h-64 w-full" /></div>
      ) : payments.length === 0 ? (
        <div className="text-center py-20">
          <div className="mx-auto w-24 h-24 rounded-full bg-muted/50 flex items-center justify-center mb-4"><CreditCard className="h-12 w-12 text-muted-foreground/50" /></div>
        ...
        <>
          ...
          <Card className="border-blue-100 bg-gradient-to-r from-blue-50/50 to-white shadow-sm">
            <CardContent className="pt-6 space-y-4">
              <div className="flex items-center justify-between flex-wrap gap-2">
                <div className="flex items-center gap-2 text-sm text-muted-foreground"><Wallet className="size-4 text-blue-600" />Résumé des paiements — {selectedChild?.fullName}</div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4">
                <div className="flex items-center gap-3 p-3 rounded-lg bg-emerald-50/80 border border-emerald-100">
                  <div className="size-9 rounded-full bg-emerald-100 flex items-center justify-center shrink-0"><CheckCircle2 className="size-4 text-emerald-600" /></div>
                  <div><p className="text-xs text-muted-foreground">Payé</p><p className="text-sm font-semibold text-emerald-700">{paidPayments.length} · {formatAmount(paidAmount)} {currency}</p></div>
                </div>
                <div className="flex items-center gap-3 p-3 rounded-lg bg-amber-50/80 border border-amber-100">
                  <div className="size-9 rounded-full bg-amber-100 flex items-center justify-center shrink-0"><Clock className="size-4 text-amber-600" /></div>
                  <div><p className="text-xs text-muted-foreground">En attente</p><p className="text-sm font-semibold text-amber-700">{pendingPayments.length} · {formatAmount(pendingAmount)} {currency}</p></div>
                </div>
                <div className="flex items-center gap-3 p-3 rounded-lg bg-red-50/80 border border-red-100">
                  <div className="size-9 rounded-full bg-red-100 flex items-center justify-center shrink-0"><AlertTriangle className="size-4 text-red-600" /></div>
                  <div><p className="text-xs text-muted-foreground">En retard</p><p className="text-sm font-semibold text-red-700">{overduePayments.length} · {formatAmount(overdueAmount)} {currency}</p></div>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="shadow-sm">
            <CardHeader>
              <CardTitle className="text-lg flex items-center gap-2"><CircleDollarSign className="size-5 text-blue-600" />Historique des paiements</CardTitle>
            ...
            <CardContent>
              <ScrollArea className="max-h-[500px]">
                <Table>
                  <TableHeader>
                    <TableRow className="bg-muted/30 hover:bg-muted/30"><TableHead><span className="flex items-center gap-1.5"><CalendarDays className="size-3.5" />Mois</span></TableHead><TableHead className="text-right"><span className="flex items-center justify-end gap-1.5"><CreditCard className="size-3.5" />Montant</span></TableHead><TableHead className="text-center">Statut</TableHead><TableHead className="hidden sm:table-cell">Méthode</TableHead><TableHead className="hidden md:table-cell">Date</TableHead></TableRow>
                  </TableHeader>
                  <TableBody>
                    {[...payments].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).map(payment => (
                      <TableRow key={payment.id} className="even:bg-muted/20 hover:bg-blue-50/50 transition-colors">
                        <TableCell className="font-medium">
                          <div className="flex items-center gap-2">{payment.status === 'overdue' && <AlertTriangle className="size-4 text-red-500 shrink-0" />}{payment.month}</div>
                        </TableCell>
