...
export default function AdminReports() {
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      // Get all grades for the school, then filter
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Analysez les performances de votre école</p>
      </div>
            ...
            <div className="mt-6 space-y-4">
              ...
              <div className="bg-white border rounded-xl p-6">
                <div className="text-center mb-6">
                  <h3 className="text-lg font-bold">{classes.find((c) => c.id === selectedClass)?.name || ''}</h3>
                ...
                {Object.keys(reportByStudent).length === 0 ? (
                  <p className="text-center text-muted-foreground py-8">
                    Aucune note disponible pour cette classe et ce trimestre
                  </p>
                ) : (
                              ...
                              {student.grades.map((g) => (
                                <TableRow key={g.id}>
                                  ...
                                  <TableCell className="text-center font-semibold">
                                    {g.maxScore > 0
                                      ? ((g.score / g.maxScore) * 20).toFixed(1)
                                      : g.score}
                                  </TableCell>
                      ...
                      return (
                        <div key={course.name} className="space-y-1">
                          <div className="flex items-center justify-between text-sm">
                            ...
                            <span className="font-semibold">{course.average.toFixed(1)} / 20</span>
                          </div>
                      ...
                      {payments.length > 0 && (
                        <>
                          <div
                            className="bg-green-500 transition-all"
                            style={{
                              width: `${(paymentStats.paid / payments.length) * 100}%`,
                            }}
                          />
                          <div
                            className="bg-yellow-500 transition-all"
                            style={{
                              width: `${(paymentStats.pending / payments.length) * 100}%`,
                            }}
                          />
                          <div
                            className="bg-red-500 transition-all"
                            style={{
                              width: `${(paymentStats.overdue / payments.length) * 100}%`,
                            }}
                          />
                        </>
                      )}
